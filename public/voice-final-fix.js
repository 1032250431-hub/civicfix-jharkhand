...
(function(){
  ...
  async function checkServer(){
    try{
      const r=await fetch("/api/voice-transcribe",{method:"POST",headers:{"Content-Type":"audio/webm"},body:new Blob([], {type:"audio/webm"}),cache:"no-store"});
      serverReady=r.status!==503;
    }catch(_){serverReady=null}
    applyMode();
  }
  function applyMode(){
    if(serverReady!==false)return;
    if(!SpeechRecognition){
      status("Voice service needs server setup. You can type the complaint below.","error");
      return;
    }
    const b=$("cfVoiceStart");if(!b)return;
    b.onclick=startBrowser;
    b.disabled=false;
    b.innerHTML='<span class="cf-voice-main"><span class="cf-voice-dot"></span><span>Start voice report</span></span>';
    status("Voice service is offline right now — using your browser voice input instead.");
  }
  function startBrowser(){
    ...
    try{
      ...
      recognition.onresult=e=>{let interim="";for(let i=e.resultIndex;i<e.results.length;i++){const text=e.results[i][0].transcript;if(e.results[i].isFinal)finalText+=text+" ";else interim+=text}const preview=$("cfVoiceText");if(preview)preview.textContent=(finalText+interim).trim()};
      ...
      recognition.onend=()=>{if(listening)finishBrowser(finalText)};
      recognition.start();
  ...
  function finishBrowser(text){
    ...
    listening=false;const r=recognition;if(r?.__timer)clearInterval(r.__timer);recognition=null;$("cfVoiceBox")?.classList.remove("recording");
    transcript=(text||$("cfVoiceText")?.textContent||"").trim();timer(0);
    if(!transcript){status("No speech was detected. Please try again.","error");setButton("Try voice again",startBrowser);return}
    if($("cfVoiceText"))$("cfVoiceText").textContent=transcript;
    if($("cfVoiceLang"))$("cfVoiceLang").textContent="Browser voice input · review before submitting";
    $("cfVoicePreview")?.classList.add("show");$("cfVoiceActions")?.classList.add("show");
    setButton("Record again",startBrowser);status("Transcript ready. Review it before submitting.","success");
  }
  function use(){
    const d=$("desc");if(!d||!transcript)return;d.value=d.value.trim()?`${d.value.trim()}\n${transcript}`:transcript;d.dispatchEvent(new Event("input",{bubbles:true}));$("cfVoicePreview")?.classList.remove("show");$("cfVoiceActions")?.classList.remove("show");status("Transcript added to the description. Please review it.","success");d.focus();
  }
  function wire(){
    if(!$("cfVoiceBox"))return;
    const useBtn=document.querySelector("#cfVoiceActions button");
    if(useBtn)useBtn.onclick=use;
    checkServer();
  }
  const obs=new MutationObserver(()=>{if($("cfVoiceBox"))wire()});
