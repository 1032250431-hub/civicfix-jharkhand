...
function buildSarvamMultipart(audio, contentType) {
  ...
  return {
    body: Buffer.concat([head, audio, middle]),
...
async function transcribeVoice(req, res) {
  ...
  if (!apiKey) {
    return sendJson(res, 503, { error: "Voice reporting is not configured on the server yet." });
  }

  const declaredLength = Number(req.headers["content-length"] || 0);
  if (declaredLength > MAX_VOICE_BYTES) {
    return sendJson(res, 413, { error: "Voice recording is too large. Please keep it short and try again." });
  }

  let audio;
  try {
    audio = await readRequestBody(req);
  } catch (err) {
    if (err.code === "LIMIT") {
      return sendJson(res, 413, { error: "Voice recording is too large. Please keep it short and try again." });
    }
  ...
  if (!audio.length) {
    return sendJson(res, 400, { error: "The recording was empty. Please try again." });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        return sendJson(res, 502, { error: "Voice service authentication failed. Please check the server API key." });
      }
      if (response.status === 429) {
        return sendJson(res, 429, { error: "Voice service is temporarily rate-limited. Please try again in a moment." });
      }
    ...
    if (err?.name === "AbortError") {
      return sendJson(res, 504, { error: "Voice transcription took too long. Please try a shorter recording." });
    }
...
function serveFile(res, file, p) {
  ...
  // Load the isolated voice feature after the existing CivicFix script.
  // This avoids modifying the existing application logic in index.html.
  if (p === "/index.html" && ext === ".html") {
    const html = body.toString("utf8");
    body = Buffer.from(html.replace("</body>", '<script src="/voice.js"></script></body>'), "utf8");
  }

  res.writeHead(200, {
    "Content-Type": mime[ext] || "application/octet-stream",
    "Cache-Control": p === "/index.html" ? "no-store" : "public,max-age=3600"
  });
  res.end(body);
  return true;
}
...
http.createServer(async (req, res) => {
  ...
  if (p === "/health" || p === "/api/health") {
    return sendJson(res, 200, { ok: true, service: "CivicFix" });
  }
